...
const GlobalDrawer = forwardRef(function GlobalDrawer(
  {
    content,
    pressRelease,
    index,
  ...
  return (
    ...
    >
      ...
      <SheetContent
        side={desktopOrLaptop ? 'right' : 'bottom'}
        onCloseAutoFocus={(event) => event.preventDefault()}
        className="z-[999] h-2/4 sm:h-full sm:max-h-full flex flex-col justify-between"
      >
        ...
        <SheetFooter className="h-auto w-full flex flex-col bg-background gap-4">
          {router.pathname.startsWith('/viewing-rooms') && (
            <div className="w-full h-fit">
              <button
                type="button"
                aria-label="Enquire with the gallery via email"
                className="group flex h-12 w-full items-center justify-center border-secondary hover:border-primary border-solid border rounded-[2px] bg-none transition"
              >
                <a
                  className="uppercase text-center pointer-events-auto transition group-hover:text-primary text-secondary"
          ...
          <div className="w-full justify-start flex">
            <div className="inline-flex gap-2.5">
              ...
              >
                <h3 className="uppercase pointer-events-auto text-secondary transition hover:text-primary">
                  {'Prev'}
                </h3>
              </button>
            ...
            <div className="w-full flex justify-end">
              {pressRelease ? (
                <button
                  type="button"
                  aria-label="Toggle to view the caption or the press release"
                  onClick={togglePressRelease}
                >
                  <h3 className="uppercase text-secondary transition hover:text-primary">
