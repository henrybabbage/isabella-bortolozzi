...
export default function MobileArtistSubNav({ artist, isLoading, scrollIntoViewWorks, scrollIntoViewExhibitions, scrollIntoViewBiography }) {
    ...
    return (
        <section className="fixed bg-background h-auto z-50 top-0 grid w-screen grid-cols-12 px-6 pt-6 pb-1 sm:pt-0 sm:pb-0">
            <div className="sm:col-span-9 sm:col-start-4 col-span-12 col-start-1 flex justify-between sm:justify-normal w-full space-x-12 pt-14 sm:pt-6">
                ...
                <SectionsPopover isLoading={isLoading} isOpen={isOpen} setIsOpen={setIsOpen} scrollIntoViewWorks={scrollIntoViewWorks} scrollIntoViewExhibitions={scrollIntoViewExhibitions} scrollIntoViewBiography={scrollIntoViewBiography} />
            </div>
        </section>
    )
}

const SectionsPopover = ({ isLoading, isOpen, setIsOpen, scrollIntoViewWorks, scrollIntoViewExhibitions, scrollIntoViewBiography }) => {
    ...
    return (
        <Popover.Root open={isOpen} onOpenChange={setIsOpen} className='z-[999] bg-background h-fit w-full sm:hidden'>
            <Popover.Trigger asChild className='shadow-transparent shadow-none focus:shadow-none outline-none focus:outline-none'>
                <button type="button" aria-label="Open menu to select section" className="h-fit px-2">
                    <h3 className="text-secondary hover:text-primary active:text-primary text-2xl align-top">{'↓'}</h3>
                </button>
            </Popover.Trigger>
            <Popover.Portal className="shadow-transparent shadow-none focus:shadow-none">
                <Popover.Content onCloseAutoFocus={(event) => event.preventDefault()} className="z-[999] shadow-transparent shadow-none focus:shadow-none outline-none focus:outline-none h-fit bg-background w-screen">
